...
export function bootEditorCuadroSinoptico() {
  if (hasBooted) return;
  hasBooted = true;
  ...
  dom.btnPasteMap.addEventListener("click", async () => {
    try {
      ...
      if (navigator.clipboard?.readText) {
        try {
          nextChart = String(await navigator.clipboard.readText()).trim();
        } catch (_) {
          nextChart = "";
        }
      }
      ...
      if (!nextChart) {
        const pastedManually = window.prompt(
          "Pega aqui el contenido de chart (dict Python o JSON).",
        ...
        nextChart = String(pastedManually).trim();
      }
      ...
      dom.conceptEditor.value = nextChart;
      setStatus("chart reemplazado. Generando preview...");
      await runtimeApi.generatePreview();
    } catch (error) {
      setStatus(`No se pudo pegar el chart: ${error.message}`, "error");
    }
  });
